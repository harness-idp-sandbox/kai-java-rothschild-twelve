...
# run app
java -jar target/java-rothschild-twelve-1.0-SNAPSHOT.jar
...
docker buildx build   --platform linux/amd64   -t parsontodd/java-rothschild-twelve:latest   --push .
```
...
# watch rollout
kubectl rollout status deploy/java-rothschild-twelve -n kaiser

# get external endpoint
kubectl get svc java-rothschild-twelve-svc -n kaiser
```
...
```bash
kubectl port-forward svc/java-rothschild-twelve-svc 8080:80 -n kaiser
...
kubectl rollout restart deploy/java-rothschild-twelve -n kaiser
kubectl rollout status deploy/java-rothschild-twelve -n kaiser
```
...
info.app.name=javarothschildtwelve
...
├─ src/
│  ├─ main/
│  │  ├─ java/com/harness/javarothschildtwelve/
│  │  │  ├─ App.java
...
│  │        └─ index.html
│  └─ test/
│     └─ java/com/harness/javarothschildtwelve/AppTest.java
├─ kubernetes/
│  ├─ deployment.yml
│  └─ service.yml
├─ Dockerfile
